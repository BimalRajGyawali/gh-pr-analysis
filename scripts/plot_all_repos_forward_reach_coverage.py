...
DEFAULT_OUT = _DEFAULT_FLOWS / "all_repos_pr_fpr_histogram_merged.png"


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Plot FPR across PRs (merged two-panel figure).")
...
def fpr_values(rows: list, defined_only: bool) -> list[float]:
    values: list[float] = []
    for r in rows:
...
def main() -> None:
    ...
    values_all = fpr_values(rows, False)
    values_def = fpr_values(rows, True)
    ...
    draw_fpr_panel(ax0, rows, values_all, False, n_total, n_nodes_pos)
    draw_fpr_panel(ax1, rows, values_def, True, n_total, n_nodes_pos)
    fig.tight_layout()

...
Output (single file, two panels: all PRs and n_nodes>0 only):
  viz_output_all_repos/flows/all_repos_pr_median_flow_histogram_merged.png
"""
...
DEFAULT_OUT = _DEFAULT_FLOWS / "all_repos_pr_median_flow_histogram_merged.png"


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Median root flow size across PRs (merged two-panel figure).")
...
def median_flow_values2(rows: list, defined_only: bool) -> list[int]:
    values2: list[int] = []
    for r in rows:
...
def draw_median_flow_panel(ax, values2: list[int], defined_only: bool) -> None:
    if not values2:
...
def main() -> None:
    ...
    values_all = median_flow_values2(rows, False)
    values_def = median_flow_values2(rows, True)
    ...
    draw_median_flow_panel(ax0, values_all, False)
    draw_median_flow_panel(ax1, values_def, True)
    fig.tight_layout()

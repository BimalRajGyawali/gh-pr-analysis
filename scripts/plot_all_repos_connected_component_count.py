#!/usr/bin/env python3
"""
Plot distribution of the number of connected components per PR (size ≥ 2 only).
...
DEFAULT_OUT = _DEFAULT_CC / "all_repos_pr_connected_component_count_histogram.png"
DEFAULT_OUT_DEFINED_ONLY = _DEFAULT_CC / "all_repos_pr_connected_component_count_histogram_defined_only.png"


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Plot connected component count per PR (size ≥ 2).")
...
def main() -> None:
    ...
    ax.set_xlabel("Components per PR")
    ax.set_ylabel("Number of PRs")
    ax.set_title(
        "PR vs. component count"
        + (" (defined-only: n_nodes>0)" if args.defined_only else "")
        + "\n(components with ≥2 nodes)"
    )

    info_lines = [
        f"PRs total: {n_total:,}",
        f"count > 0: {n_gt0:,} ({pct(n_gt0, n_total):.1f}%)",
    ]
    for t in thresholds[1:]:
        info_lines.append(f"count ≥ {t}: {n_ge[t]:,} ({pct(n_ge[t], n_total):.1f}%)")
